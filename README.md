# Portfolio

#### 3 july 2026
- [x] TimeLine for Experience section using gsap
- ![image](Documentation/ScreenShots/10.png)
- [x] Tech stack 3d models
- ![image](Documentation/ScreenShots/11.png)

#### 2 july 2026
- [x] UI for ablities and started experience section
- ![image](Documentation/ScreenShots/9.png)

#### 1 july 2026
- [x] showcase and Navbar
- ![image](Documentation/ScreenShots/7.png)
- [x] marquee using react-icons
- ![image](Documentation/ScreenShots/8.png)

#### 30 june 2026
- [x] reading particals of character
- ![image](Documentation/ScreenShots/5.png)
- [x] basic UI for showcase section
- ![image](Documentation/ScreenShots/6.png)

#### 29 june 2026
- [x] using threejs to make basic shapes
- ![image](Documentation/ScreenShots/3.png)
- [x] using gltfjs to make .tsx from .glt
- ![image](Documentation/ScreenShots/4.png)

#### 28 june 2026
- [x] Created a new project
- ![image](Documentation/ScreenShots/1.png)
- [x] hero text
- ![image](Documentation/ScreenShots/2.png)

